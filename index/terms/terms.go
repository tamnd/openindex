// Package terms is the segment term dictionary (indexer doc 05.4): it maps a
// term to its posting-list pointer and statistics and supports ordered
// enumeration for prefix and range scans.
//
// In production this is a Finite State Transducer per field (Vellum, the
// Lucene/Tantivy model) built by Daciuk incremental minimal-acyclic
// construction over the segment's already-sorted terms, fronting an on-disk
// BlockTree. That machinery is the swap target; the seam is the Dictionary
// interface, and SortedDict is the in-memory reference it is tested against. The
// two properties the rest of the engine leans on are pinned here: ordered
// enumeration (so prefix/range scans are a contiguous walk, doc 07 query
// expansion) and a negative lookup that needs no posting-store seek.
package terms
...
type Entry struct {
	DocFreq      int
	Singleton    bool
	SingletonDoc openindex.DocID
	// SingletonFreq is the term frequency in the single document, carried inline
	// so the SingletonDocID optimization preserves scoring inputs, not just the
	// doc id; meaningful only when Singleton.
	SingletonFreq uint32
	// PostingOffset locates the term's encoded posting list in the segment's
...
type Dictionary interface {
	...
	// PrefixScan calls fn for every term with the given prefix, in ascending
	// term order, stopping early if fn returns false.
