...
// Block-Max WAND loop can read a block's bound without decoding it (doc 05.3).
type block struct {
	firstDoc openindex.DocID // absolute id of the block's first doc, for skip
	lastDoc  openindex.DocID // absolute id of the block's last doc (skip target)
...
// term's global score upper bound in the WAND retrieval loop (doc 05.3).
...
// (doc 05.2). It returns an error on unsorted or duplicate input, since a
// posting list with out-of-order docs would silently corrupt intersection.
func Encode(ps []openindex.Posting) (*List, error) {
	for i := 1; i < len(ps); i++ {
		if ps[i].Doc <= ps[i-1].Doc {
			return nil, errors.New("postings: input not strictly ascending by DocID")
		}
	}
...
func (l *List) encodeBlock(ps []openindex.Posting) {
	b := block{
		firstDoc: ps[0].Doc,
...
type Cursor struct {
	...
	docs     [BlockSize]openindex.DocID
	freqs    [BlockSize]uint32
	decoded  bool
	// shallowIdx is the block pointer used by AdvanceShallow/BlockMax* only. It
	// is kept separate from blockIdx so reading a block's bound never disturbs
	// the decode/iteration position (the Lucene shallow-vs-deep advance split).
	shallowIdx int
}
...
func (c *Cursor) Doc() openindex.DocID { return c.docs[c.inBlock] }
...
// block; this reads its bound without decoding the block payload (doc 05.3).
...
func (c *Cursor) BlockLastDoc() openindex.DocID { return c.l.blocks[c.shallowIdx].lastDoc }
...
// the block is worth a full decode (doc 05.3).
func (c *Cursor) AdvanceShallow(target openindex.DocID) {
...
func (c *Cursor) decodeBlock() {
	...
	for i := 1; i < b.count; i++ {
		c.docs[i] = c.docs[i-1] + openindex.DocID(gaps[i-1])
	}
