// Package bm25f is the lexical relevance scorer (architecture doc 07.2): BM25F,
// the fielded variant of BM25. It scores a query term against a document from
// the term's per-field frequencies and the document's per-field lengths, using
// a single weighted pseudo-frequency across fields rather than a sum of
// independent per-field BM25 scores.
//
// The distinction matters. Summing per-field BM25 lets a term saturate
// separately in each field, so a term stuffed into the title saturates the
// title contribution while still earning full body credit, which breaks the
...
// separately; k1 is shared. Anchor text and title carry high field weights.
//
// BM25 is ambiguous across implementations (the IDF form, the (k1+1) factor,
// and the query-term weighting all vary), so the exact variant is pinned here
// and is meant to be recorded with each index version for reproducibility.
package bm25f
...
// weights lift title and anchor text, which are short and high-signal, above
// the body.
const (
	DefaultK1 = 1.2
	DefaultB  = 0.75
...
// Params is the pinned scorer configuration. It is recorded with the index
// version so a score is reproducible (doc 07.2).
type Params struct {
...
func DefaultParams() Params {
	...
	p.Weights[openindex.FieldBody] = 1.0
	p.Weights[openindex.FieldTitle] = 3.0
	p.Weights[openindex.FieldAnchor] = 2.0
	p.Weights[openindex.FieldURL] = 1.5
	return p
}
...
type Collection struct {
	...
	AvgFieldLen [openindex.NumFields]float32
}
...
// non-negative, which is the precondition Block-Max WAND places on the scorer
// (doc 05.3). A df above N is clamped to N.
...
// document. It is the relevance-stage scorer and the source of the BM25F
// feature in the LTR vector (doc 07.5).
type Scorer struct {
	params Params
	col    Collection
}

// New returns a Scorer for the given params and collection. A zero average
// field length is replaced by 1 so the length ratio is well defined for a field
// that happens to have no observed content.
func New(p Params, c Collection) *Scorer {
...
func (s *Scorer) pseudoFreq(tf, fieldLen [openindex.NumFields]uint32) float32 {
	var eff float32
	for f := range openindex.NumFields {
...
// pseudo-frequency.
func (s *Scorer) saturate(idf, eff float32) openindex.Score {
	if eff <= 0 {
		return 0
	}
	k1 := s.params.K1
	return openindex.Score(idf * eff * (k1 + 1) / (k1 + eff))
}
...
func (s *Scorer) ScoreTerm(idf float32, tf, fieldLen [openindex.NumFields]uint32) openindex.Score {
...
type TermStats struct {
	IDF float32
	TF  [openindex.NumFields]uint32
}

// Score sums the per-term BM25F contributions for a document whose per-field
...
func (s *Scorer) Score(terms []TermStats, fieldLen [openindex.NumFields]uint32) openindex.Score {
	var total openindex.Score
	for i := range terms {
